...
BEGIN 
  ...
  CREATE TEMPORARY TABLE temp_all_tasks ON COMMIT DROP AS 
    ...
    WHERE (t.emp_id = p_emp_id OR t.emp_task_id IN ( 
      SELECT pc.emp_task_id FROM process_config pc WHERE pc.emp_id = p_emp_id AND pc.emp_task_id IS NOT NULL AND (pc.is_live = true OR pc.is_live IS NULL)
...
BEGIN 
  ...
  ( 
    SELECT 
      ...
      false AS is_individual, 
      COALESCE( 
        (SELECT pc.emp_id FROM process_config pc WHERE pc.task_id = t.task_id AND pc.is_live = true AND pc.ordr_id = 1 LIMIT 1), 
        NULL 
      ) AS reviewer_id, 
      COALESCE( 
        (SELECT pc.emp_id FROM process_config pc WHERE pc.task_id = t.task_id AND pc.is_live = true AND pc.ordr_id = 2 LIMIT 1), 
        NULL 
      ) AS approver_id, 
      COALESCE( 
    ...
    LEFT JOIN task_status_master tsm ON tsm.status_id = t.task_sts 
    LEFT JOIN task_priority_master pm ON pm.priority_id = t.priority 
    WHERE (t.emp_id = p_emp_id OR t.task_id IN (SELECT pc.task_id FROM process_config pc WHERE pc.emp_id = p_emp_id AND pc.is_live = true))
    ...
    SELECT 
      ...
      true AS is_individual, 
      COALESCE( 
        (SELECT pc.emp_id FROM process_config pc WHERE pc.emp_task_id = t.emp_task_id AND (pc.is_live = true OR pc.is_live IS NULL) AND pc.ordr_id = 1 LIMIT 1), 
        NULL 
      ) AS reviewer_id, 
      COALESCE( 
        (SELECT pc.emp_id FROM process_config pc WHERE pc.emp_task_id = t.emp_task_id AND (pc.is_live = true OR pc.is_live IS NULL) AND pc.ordr_id = 2 LIMIT 1), 
        NULL 
      ) AS approver_id, 
      COALESCE( 
    ...
    LEFT JOIN task_status_master tsm ON tsm.status_id = t.task_sts 
    LEFT JOIN task_priority_master pm ON pm.priority_id = t.priority 
    WHERE COALESCE(t.sts, true) = true AND (t.emp_id = p_emp_id OR t.emp_task_id IN (SELECT pc.emp_task_id FROM process_config pc WHERE pc.emp_id = p_emp_id AND pc.emp_task_id IS NOT NULL AND (pc.is_live = true OR pc.is_live IS NULL)))
  ); 
...
async function main() {
  ...
  console.log("Updating get_user_dashboard...");
  await dbClient.query(sqlGetUserDashboard);
  console.log("Updating get_my_tasks_data...");
  await dbClient.query(sqlGetMyTasksData);
  console.log("Updating get_user_my_tasks...");
  await dbClient.query(sqlGetUserMyTasks);
  console.log("All procedures successfully updated in database!");
  await dbClient.end();
}

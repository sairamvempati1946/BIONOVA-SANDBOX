...
async function main() {
  ...
  console.log('--- Testing progress calculation for Emp 5 ---');
    ...
    SELECT 
      ...
      COALESCE(ROUND(
        (COUNT(t_all.task_id) FILTER (WHERE UPPER(tsm_all.status_nm) IN ('COMPLETED', 'CLOSED'))::NUMERIC 
         / NULLIF(COUNT(t_all.task_id), 0)) * 100, 0), 0) AS calc_progress
  ...
  console.log('--- Testing progress calculation for Emp 7 ---');

    ...
    SELECT 
      ...
      COALESCE((
        ...
        WHERE ml_all.prj_id = p.prj_id
          AND (t_all.emp_id = 7 OR t_all.task_id IN (SELECT pc.task_id FROM process_config pc WHERE pc.emp_id = 7))
          AND COALESCE(UPPER(tsm_all.status_nm), '') <> 'DRAFT'
      ), 0) AS unweighted_progress
    FROM project_live_master p
    WHERE p.prj_id IN (24, 6, 12, 19)
    ORDER BY p.prj_nm;
  `);
  console.log('Unweighted progress per project for Emp 7:');

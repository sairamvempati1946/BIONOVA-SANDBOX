...
async function main() {
  ...
  console.log('--- Emp 5 Project Progress Comparison ---');
  const res5 = await dbClient.query(`
    SELECT 
      ...
      COALESCE(ROUND(
        (COUNT(t_all.task_id) FILTER (WHERE UPPER(tsm_all.status_nm) IN ('COMPLETED', 'CLOSED'))::NUMERIC 
         / NULLIF(COUNT(t_all.task_id), 0)) * 100, 0), 0) AS pure_closed_pct,
      COALESCE(ROUND(
        (SUM(
          CASE 
            ...
            ELSE 0.0
          END
        ) / NULLIF(COUNT(t_all.task_id), 0)) * 100, 0), 0) AS fractional_pct
    ...
    WHERE (t_all.emp_id = 5 OR t_all.task_id IN (SELECT pc.task_id FROM process_config pc WHERE pc.emp_id = 5))
  ...
  console.log('--- Emp 7 Project Progress Comparison ---');
  const res7 = await dbClient.query(`
    ...
    JOIN task_live_master t_all ON t_all.m_id = ml_all.m_id
    LEFT JOIN task_status_master tsm_all ON tsm_all.status_id = t_all.task_sts
    WHERE (t_all.emp_id = 7 OR t_all.task_id IN (SELECT pc.task_id FROM process_config pc WHERE pc.emp_id = 7))
      AND COALESCE(UPPER(tsm_all.status_nm), '') <> 'DRAFT'
      AND p.prj_sts IN ('LIVE', 'CLOSED', 'HOLD')
    GROUP BY p.prj_id, p.prj_cd, p.prj_nm
    ORDER BY p.prj_nm;
